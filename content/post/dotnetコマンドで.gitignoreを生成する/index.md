---
title: 'dotnetコマンドで.gitignoreを生成する'
...
## .NET SDKのインストール
dotnet.exeは.NET SDKに含まれているので、まずは.NET SDKをインストールします。
管理者権限でcmd.exeを起動し、以下のコマンドを実行します。
```cmd
winget install Microsoft.DotNet.SDK.6
```

## .gitignoreを生成する
dotnet.exeで.gitignoreを生成するコマンドは以下の通りです。
```bash
dotnet new gitignore
```
生成された.gitignoreは以下の通りです。
```gitignore
